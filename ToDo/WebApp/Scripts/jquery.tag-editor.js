...
(function($) {
	...
	$.fn.tagEditor = function(options, val, blur) {
			...
			selector.each(function() {
				...
				if (options == "getTags")
					...
					$('<li><div class="tag-editor-spacer">&nbsp;' +
							o.delimiter[0] +
							'</div><div class="tag-editor-tag"></div><div class="tag-editor-delete"><i></i></div></li>').appendTo(ed)
						.find(".tag-editor-tag")
					...
					$(".tag-editor-tag", ed).filter(function() { return $(this).text() == val; }).closest("li")
						.find(".tag-editor-delete").click();
					if (!blur) ed.click();
		...
		if (window.getSelection)
			$(document).off("keydown.tag-editor").on("keydown.tag-editor",
				function(e) {
					if (e.which == 8 || e.which == 46 || e.ctrlKey && e.which == 88) {
						try {
							var sel = getSelection(),
								el = document.activeElement.tagName == "BODY"
		...
		return selector.each(function() {
			...
			var ed = $("<ul " + (o.clickDelete ? 'oncontextmenu="return false;" ' : "") + 'class="tag-editor"></ul>')
				.insertAfter(el);
			...
			// markup for new tag
			var new_tag = '<li><div class="tag-editor-spacer">&nbsp;' +
				o.delimiter[0] +
			...
			ed.on("click",
				...
				function(e) {
					...
					tag.addClass("deleted").animate({ width: 0 },
						o.animateDelete,
						function() {
							li.remove();
							set_placeholder();
					...
					if (!$(this).hasClass("active")) {
						...
						if (tag == "") {
							...
							var left_percent = Math.abs(($(this).offset().left - e.pageX) / $(this).width()),
								...
								input = $(this).html('<input type="text" maxlength="' + o.maxLength + '" value="' + escape(tag) + '">')
			...
			function split_cleanup(input) {
				...
				for (var i = 0; i < sub_tags.length; i++) {
					...
					old_tags.push(tag);
					li.before('<li><div class="tag-editor-spacer">&nbsp;' +
						o.delimiter[0] +
				...
				function(e) {
					...
					if ((e.which == 37 || !o.autocomplete && e.which == 38) && !$t.caret() || e.which == 8 && !$t.val()) {
						...
						if (prev_tag.length) prev_tag.click().find("input").caret(-1);
							...
							!(o
								.maxTags &&
								ed.data("tags").length >= o.maxTags)) $(new_tag).insertBefore($t.closest("li")).find(".tag-editor-tag").click();
						return false;
					}
					// right/down key
			...
			for (var i = 0; i < tags.length; i++) {
				...
				if (tag) {
					if (o.forceLowercase) tag = tag.toLowerCase();
					tag_list.push(tag);
					ed.append('<li><div class="tag-editor-spacer">&nbsp;' +
						o.delimiter[0] +
						'</div><div class="tag-editor-tag">' +
						escape(tag) +
						'</div><div class="tag-editor-delete"><i></i></div></li>');
